...
class ServerDB:
    ...
    def initDb(self):
        if start_flg == 1 and self.point > 2:
            ...
            try:
                stmt = "insert into user_history (user_id,oc_id,`point`,remain_point,operation,op_user_id,op_raspi_id,op_date,session_id,json_id) values (?,?,?,?,1,null,?,now(),?,?)" # mekapit
                cur.execute(stmt, (userid, ocid, 0, self.point, RP_ID, sess, jsonkey)) # mekapit
                conn.commit()
...
# --------------------------------
@webiopi.macro
def Game_Initialize(u, o, s): # mekapit
    global sess
    global userid
    global ocid # mekapit
    global credit
